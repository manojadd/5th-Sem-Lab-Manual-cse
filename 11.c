...
{
 int n,a[100],i;
 omp_set_num_threads(2); //max threads that can be spawned
 ...
 {
  #pragma omp single //only single thread
  for(i=2;i<n;i++)
  {
   a[i]=a[i-2]+a[i-1];
   printf("id of thread involved in the computation of fib no %d is=%d\n",i+1,omp_get_thread_num());
  }
  #pragma omp barrier// wait till all spawned threads have completed
  #pragma omp single
  {
